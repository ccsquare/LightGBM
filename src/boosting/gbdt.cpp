...
void GBDT::Init(const BoostingConfig* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
  ...
  if (gbdt_config_->bagging_fraction < 1.0 && gbdt_config_->bagging_freq > 0) {
    ...
    double average_bag_rate = gbdt_config_->bagging_fraction / gbdt_config_->bagging_freq;
    int sparse_group = 0;
...
void GBDT::ResetConfig(const BoostingConfig* config) {
  ...
  if (gbdt_config_ != nullptr && gbdt_config_->bagging_fraction != new_config->bagging_fraction) {
    // if need bagging, create buffer
